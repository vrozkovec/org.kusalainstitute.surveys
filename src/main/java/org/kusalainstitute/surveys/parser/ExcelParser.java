...
/**
 * Base class for parsing Excel survey files.
 * Provides common utilities for cell value extraction.
 */
public abstract class ExcelParser {
    ...
    /**
     * Gets an integer confidence level value from a cell.
     * Parses French confidence labels to numeric values (1-4).
     *
     * @param row       the row
     * @param cellIndex the cell index (0-based)
    ...
    protected boolean isHeaderRow(Row row) {
        ...
        if (firstCell != null && (firstCell.toLowerCase().contains("column") ||
                firstCell.toLowerCase().contains("pre") ||
                firstCell.toLowerCase().contains("post"))) {
            return true;
        }
        if (secondCell != null && (secondCell.toLowerCase().contains("column") ||
                secondCell.equalsIgnoreCase("timestamp"))) {
            return true;
        }
        return false;
    }
