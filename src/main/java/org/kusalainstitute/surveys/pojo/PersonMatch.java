...
/**
 * Represents a match between a pre-survey person and a post-survey person. Matching is done within
 * the same cohort only.
 */
public class PersonMatch
{
	 ...
	 * @param matchType
	 *            type of match (AUTO_EMAIL, AUTO_NAME, MANUAL)
	 * @param confidence
	 *            confidence score (0.00-1.00) for fuzzy matches
	 */
	public PersonMatch(String cohort, Long prePersonId, Long postPersonId, MatchType matchType, BigDecimal confidence)
	{
		this();
		this.cohort = cohort;
	...
	{
		return "PersonMatch{" + "id=" + id + ", cohort='" + cohort + '\'' + ", prePersonId=" + prePersonId + ", postPersonId="
			+ postPersonId + ", matchType=" + matchType + ", confidence=" + confidence + '}';
	}
}
